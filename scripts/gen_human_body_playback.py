"""Generate human-body formation playback for static Netlify hosting."""
...
N_DRONES = 70
OUTPUT = "web/public/data/human-body-playback.json"

with open("web/public/data/default-obj-points.json") as f:
    obj_data = json.load(f)

pts = np.array(obj_data["points"], dtype=np.float64)
indices = np.unique(np.linspace(0, pts.shape[0] - 1, N_DRONES, dtype=int))
sampled_pts = pts[indices].tolist()

config = SwarmConfig(
    n_drones=len(sampled_pts),
    duration=10.0,
    ...
    primitive_params={"delta_height": 0.3, "spacing": 0.5, "t_form": 0.5, "rotation": 0.3},
    obj_points=sampled_pts,
)

print(f"Running human-body simulation: {config.n_drones} drones, {config.duration}s...")
